$(function(){
	...
	function showUsers(pageNo,pageSize)
	{
		...
		$.post("user/show/allUsers",{"pageNo":pageNo,"pageSize":pageSize},function(result){
			...
			var users = result.pageList;
			findNumReality = users.length;
			console.log("findNumReality:"+findNumReality);
			//$("#user_page ul").html("");
			$("#allUsers tbody").html("");
			$.each(users,function(n,user){
				...
				var content = "<tr>" +
						...
						"<td width='8%'><a href='#show_userext_modal' data-target='#show_userext_modal' data-toggle='modal' class='btn btn-sm btn-success detail'>详细</a><button type='button' class='btn btn-sm btn-danger delete'>删除</button></td>" +
						"</tr>"
