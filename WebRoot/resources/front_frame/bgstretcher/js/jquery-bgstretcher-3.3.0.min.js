...
(function(e) {
	...
	e.fn.bgStretcher = function(t) {
		var n = l, r = e(this), o = function() {
			...
			e(window).unbind("resize.bgstretcher").bind(
					"resize.bgstretcher",
					function() {
						e(".bgstretcher-container").each(
								function(t, n) {
									var r = e(n).data("instance");
									h(e(n), r.settings);
									r.settings.transitionEffect == "superSlide"
											&& e(n).find("UL").css({
												top : 0
											})
								})
			...
			i.find("LI").eq(r.settings.startElementIndex).addClass(
					"bgs-current");
			if (r.settings.transitionEffect === "simpleSlide") {
				...
				if (r.settings.slideDirection === "NW"
						|| r.settings.slideDirection === "NE")
					r.settings.slideDirection = "N";
				if (r.settings.slideDirection === "SW"
						|| r.settings.slideDirection === "SE")
					r.settings.slideDirection = "S";
				if (r.settings.slideDirection === "S"
						|| r.settings.slideDirection === "E") {
					r.settings.sequenceMode = "back";
					e(c).removeClass("bgs-current");
					e(c).eq(e(c).length - r.settings.startElementIndex - 1)
							.addClass("bgs-current");
					if (r.settings.slideDirection === "E") {
						f = e(c).index(i.find("LI.bgs-current")) * i.width()
								* -1;
						l = 0
					} else {
						l = e(c).index(i.find("LI.bgs-current")) * i.height()
								* -1;
						f = 0
					}
					i.find("UL").css({
					...
					if (r.settings.startElementIndex !== 0) {
						if (r.settings.slideDirection === "N") {
							l = e(c).index(i.find("LI.bgs-current"))
									* e(i).height() * -1;
							f = 0
						} else {
							f = e(c).index(i.find("LI.bgs-current"))
									* e(i).width() * -1;
							l = 0
						}
						i.find("UL").css({
			...
			if (e(r.settings.buttonNext).length
					|| e(r.settings.buttonPrev).length
					|| e(r.settings.pagination).length)
				if (r.settings.sequenceMode === "random")
					...
					if (e(r.settings.buttonPrev).length) {
						e(r.settings.buttonPrev).addClass(
								"bgStretcherNav bgStretcherNavPrev");
						e(r.settings.buttonPrev).click(function() {
							r.bgStretcher.buttonSlide(r, "prev")
						})
					}
					if (e(r.settings.buttonNext).length) {
						e(r.settings.buttonNext).addClass(
								"bgStretcherNav bgStretcherNavNext");
						e(r.settings.buttonNext).click(function() {
							r.bgStretcher.buttonSlide(r, "next")
						})
					}
					e(r.settings.pagination).length
							&& r.bgStretcher.pagination(r)
				}
			...
			if (n.sequenceMode === "random") {
				...
				n.transitionEffect !== "simpleSlide"
						&& (r.settings.startElementIndex = d);
				o = d
			} else {
				r.settings.startElementIndex > e(c).length - 1
						&& (r.settings.startElementIndex = 0);
				o = r.settings.startElementIndex;
				n.transitionEffect === "simpleSlide"
						&& (r.settings.slideDirection === "S" || r.settings.slideDirection === "E")
						&& (o = e(c).length - 1 - r.settings.startElementIndex)
			}
			...
			if (r.settings.slideShow && e(c).length > 1 && !r.settings.isPaused) {
				r.settings.displayFirstImageOnce == 1
						&& i.find("LI.bgs-current").addClass("once");
				s[n.containerId] = setTimeout(function() {
					r.bgStretcher.slideShow(r, r.settings.sequenceMode, -1)
				}, n.nextSlideDelay)
			}
		}, h = function(t, n) {
			var s = 0, o = 0, a = 0, f = 0, l = 0, c = 0, h = e(t).find(
					".bgstretcher");
			...
			n.maxHeight !== "auto" ? f = o > n.maxHeight ? n.maxHeight : o
					: f = o;
			h.width(a).height(f);
			...
			if (n.anchoring !== "left top") {
				y = n.anchoring.split(" ");
				if (y.length === 2) {
					y[0] === "right" ? m = s - a : y[0] === "center"
							&& (m = Math.round((s - a) / 2));
					y[1] === "bottom" ? g = o - f : y[1] === "center"
							&& (g = Math.round((o - f) / 2));
					h.css({
						marginLeft : m + "px",
						marginTop : g + "px"
			...
			if (n.anchoringImg !== "left top") {
				...
				if (y.length === 2) {
					y[0] === "right" ? m = a - l : y[0] === "center"
							&& (m = Math.round((a - l) / 2));
					y[1] === "bottom" ? g = f - c : y[1] === "center"
							&& (g = Math.round((f - c) / 2))
				}
			}
			i.marginLeft = m;
			...
			if (t.find("LI:first").hasClass("swf-mode"))
				if (typeof SWFObject == "undefined")
					...
					var b = e("#" + t.settings.containerId).find("LI:first")
							.html();
					e("#" + t.settings.containerId).find("LI:first").html(
							'<div id="bgstretcher-flash">&nbsp;</div>');
			...
			if (r.settings.transitionEffect === "simpleSlide")
				if (r.settings.slideDirection === "W"
						|| r.settings.slideDirection === "E") {
					h.children("UL").width(h.width() * h.find("LI").length)
							.height(h.height());
					h.find("LI").index(h.find("LI.bgs-current")) !== -1
							&& h.children("UL").css(
									{
										left : h.find("LI").index(
												h.find("LI.bgs-current"))
												* h.width() * -1 + "px"
									})
				} else {
					h.children("UL").height(h.height() * h.find("LI").length)
							.width(h.width());
					h.find("LI").index(h.find("LI.bgs-current")) !== -1
							&& h.children("UL").css(
									{
			...
			t
					.each(function() {
						e(this).wrapInner('<div class="bgstretcher-page" />')
								.wrapInner('<div class="bgstretcher-area" />');
						n = '<div id="' + t.settings.containerId
						...
						if (t.settings.images.length > 0) {
							...
							if (r !== "swf") {
								...
								for (var s = 0; s < t.settings.images.length; s++) {
									"simpleSlide" === t.settings.transitionEffect
											&& "back" === t.settings.sequenceMode ? i = t.settings.images.length
											- 1 - s
											: i = s;
									
									//宣传图片所对应的文章链接
									var url = t.settings.url[i]===null||t.settings.url[i]===''?'javascript:;':t.settings.url[i];
									//链接类型(当前窗口打开-self/新窗口打开-_bank)
									var urlType = t.settings.urlType[i];
									!0 === t.settings.preloadImages ? n += '<li><span class="image-path">'
											+ t.settings.images[i]
											+ "</span></li>" 
											: n += '<li class="img-loaded"><a href="'+url+'" target="'+urlType+'" ><img src="'
													+ t.settings.images[i]
													+ '" alt="" /></a></li>';
								}
							} else
								n += '<li class="swf-mode">'
										+ t.settings.images[0] + "</li>";
						}
						n += "</ul></div>";
						var o = e(this).children(".bgstretcher-area");
						e(n).prependTo(o);
						t.settings.content.length > 0
								&& t.settings.content.length == t.settings.images.length
								&& e(this)
										.append(
												'<div class="bgs-description-pane">'
														+ t.settings.content[t.settings.startElementIndex]
														+ "</div>");
						o.css({
							position : "relative"
						});
	...
	e.fn.bgStretcher.slideShow = function(n, i, o) {
		var u = e(n).data("instance").settings, a = "#" + u.containerId, f = e(
				a).find("LI");
		if (f.length < 2)
			return !0;
		...
		if (o === -1)
			...
			c = f.eq(o);
		u.content.length > 0
				&& e("> .bgs-description-pane", n).html(u.content[c.index()]);
		...
		if (e(u.pagination).find("LI").length) {
			...
			e(u.pagination).find("LI").eq(f.index(e(a).find("LI.bgs-current")))
					.addClass("showPage")
		}
		u.sliderCallbackFunc && typeof u.sliderCallbackFunc == "function"
				&& u.sliderCallbackFunc.call();
		u.isPaused
				|| (s[u.containerId] = setTimeout(function() {
					c.parent().find(".once").length
							&& c.parent().find(".once").remove();
	...
	e.fn.bgStretcher.pagination = function(t) {
		...
		if (r > 0) {
			i += "<ul>";
			for (s = 0; s < r; s++)
				i += '<li class="page-' + (s + 1) + '"><a href="javascript:;">'
						+ (s + 1) + "</a></li>";
			i += "</ul>";
			e(o.pagination).html(i);
			e(o.pagination).find("LI:first").addClass("showPage");
			e(o.pagination).find("A").click(
					function() {
						if (e(this).parent().hasClass("showPage"))
							return !1;
						n.stop(!0, !0);
						e(t).bgStretcher.clearSliderTimeout(t);
						e(t).bgStretcher.slideShow(t, o.sequenceMode, e(
								o.pagination).find("A").index(e(this)));
						return !1
					})
		}
		return !1
